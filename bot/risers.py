...
def sosedi_po_shahmatke(address: str) -> list:
    """Другие адреса, которые считаются по той же шахматке.

    В исходной таблице «СТОЯКИ квартир» одна схема стоит сразу на несколько
    домов — видимо, потому что планировка у них повторяется. Но дома при
    этом разные: 4-я Советская 28, Седова 71 и Седова 71/1 стоят в ста
    метрах друг от друга и заявки собирают каждый свои. Если планировки
    где-то разошлись, стояк посчитается по чужой схеме, а список квартир
    уедет жильцам. Поэтому там, где схема общая, Люся об этом предупреждает.
    """
    b, addr = find_block(address)
    if not b:
        return []
    return [a for a in b['addresses'] if a != addr]
