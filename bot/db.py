...
def set_recording(chat_id, on: bool):
    """Разрешить или запретить Люсе вести ленту этого чата.

    Заказчик выключил ленту в чате бригады, потому что одни и те же заявки
    обсуждают и там, и в «Обслуживании»: «она просто заявки оттуда собирала,
    которые ещё и в обслуживании были». В таком чате сообщение не ложится в
    ленту и видео не расшифровывается.

    Находок это не касается: «информацию сохранить какую-то по находкам —
    это нужно и в этом чате, и в том, и в личке тоже». Они пишутся всегда,
    а повтор из второго чата отсекает flat_note_exists.
    """
    with _conn() as c:
