...
# Дословно — пока отчёт помещается в сообщение. Пересказ нужен только для
# длинной серии, и он же — единственное место, где что-то может исказиться
VERBATIM_LIMIT = 500


async def short_summary(parts: list[str], address: str | None) -> str:
    """Собирает ответ на видеоотчёт: что было сказано.

    Пересказ модели заказчик поймал на выдумке: он сказал «подтапливает по
    стояку», а в чате появилось «устранена течь в потолке, заменена
    неисправная арматура». Модель достраивает историю до складной, и в
    рабочем чате это опаснее, чем длинный текст. Поэтому короткий отчёт
    отдаём дословно, а пересказываем только то, что иначе не помещается —
    и прямо помечаем как пересказ.
    """
    parts = [p.strip() for p in parts if p and p.strip()]
    # Серию показываем по роликам: это шаги работы, а не один рассказ
    ...
    if len(joined) <= VERBATIM_LIMIT:
        return f'{head}\n{joined}{hvost}'

    slova = ' '.join(parts)
    task = ('Сантехник рассказывает с объекта. Вот расшифровка его слов'
            + (' (несколько видео подряд, по порядку)' if len(parts) > 1 else '')
            + f':\n\n{slova}\n\n'
            'Сократи до 2–4 строк, СТРОГО по сказанному. Ничего не добавляй '
            'от себя: если о причине, материалах или результате не сказано — '
            'не пиши о них. Не превращай «подтапливает» в «устранено», '
            '«посмотрю» в «сделал». Только сокращай, не додумывай. '
            'Без вступлений и без адреса.')
    summary = await ai.ask(task, max_tokens=250, temperature=0)
    if not summary:
        return f'{head}\n{slova[:400]}…{hvost}'
    return (f'{head} · пересказ\n{summary.strip()}\n'
            f'📄 Дословно — в расшифровке, команда /chat.{hvost}')
