...
def _otchet_lines(stats: dict) -> list:
    ...
    # Про забытый стояк человек должен узнать сам, глазами, а не из разбора
    # модели: напоминание о нём уходит один раз, через четыре часа, и если
    # его пропустили — больше о стояке не скажет никто
    otkrytye = db.open_shutoffs()
    if otkrytye:
        lines += ['', f'🚫 Перекрыты и не открыты — {len(otkrytye)}:']
        for z in otkrytye:
            dom = houses.HOUSES_BY_ID.get(z['house_id'])
            lines.append(f"   • {dom['address'] if dom else '—'}, кв. {z['flat']} — "
                         f"{stoyak_mod.dlitelnost(_minut_s(z['closed_at']))}, "
                         f"{z['by_name'] or '—'}")
    return lines
...
OTCHET_RAZBOR = (
    ...
    'две-три рекомендации, что сделать дальше. Живым деловым языком.\n'
    'Правила, нарушать нельзя:\n'
    '— начинай сразу с сути. Ни приветствий, ни обращений, ни «вот что я '
    'вижу» — первая же строка должна быть наблюдением;\n'
    '— только то, что есть в данных. Ни домов, ни квартир, ни работ, ни '
    'причин, которых тут нет, не придумывай. Адрес пиши ровно так, как он '
    'стоит в данных, до последней буквы и дроби;\n'
    '— наблюдение — это то, что из цифр следует, а не сами цифры. Список '
    'домов человек видит выше: заново перечислять, где сколько сообщений, '
    'не нужно. Ценно другое — что повторяется, где копится аварийное, что '
    'висит незакрытым неделями, что одно с другим связано;\n'
    '— каждая рекомендация опирается на строку из данных и говорит, что '
    'сделать. Общие советы вроде «усилить контроль» и «наладить учёт» не '
    'пиши вовсе;\n'
    '— не оправдывайся нехваткой данных и не проси прислать ещё: разбирай '
    'то, что дано. Нечего сказать — лучше две точные строки, чем пять '
    'пустых;\n'
