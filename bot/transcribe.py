...
# Примеров с настоящими адресами здесь быть не должно. Стояло «квартира 47»,
# «Байкальская 237» — и модель переписала их в расшифровку как услышанное:
# в отчёте по Красных Мадьяр появились две заявки по чужому дому
PROMPT = ('Это рабочий отчёт сантехника управляющей компании. Расшифруй речь '
          'дословно на русском языке. Числа — номера домов, квартир, '
          'подъездов, этажей, показания приборов — записывай цифрами, а не '
          'словами. Пиши только то, что сказано, без комментариев и пояснений. '
          'Ничего не добавляй от себя: ни адресов, ни номеров квартир, ни '
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(f'{ai.OPENROUTER_BASE_URL}/chat/completions',
                ...
                if not text:
                    # Запрос прошёл, а расшифровки нет — модель сама решила,
                    # что речи не разобрать (шум улицы, тихий голос). Без
                    # этой строки такой случай в логе неотличим от того, что
                    # запрос вообще не ушёл — искать причину пришлось бы
                    # заново каждый раз
                    log.info('Модель не расслышала речь — пустая расшифровка')
                    return None
