...
# Находка называется термином, а не тем словом, которое прозвучало:
# «течка» в отчёте недопустима, «засорён» — не существительное
@pytest.mark.parametrize('text,chto', [
    ('нашёл подмес воды', 'подмес'),
    ('течь под мойкой', 'течь'),
    ('течка под мойкой', 'течь'),
    ('кран не перекрывается', 'запорная арматура не держит'),
    ('стояк засорён', 'засор'),
])
